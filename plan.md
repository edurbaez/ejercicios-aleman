...
## ✅ RESUELTO — Adaptar plan.js a las reglas A1/A2 ampliadas (2026-07-20)

`GRAMMAR_DATA.A1` pasó de 10 a **21 reglas** (`a1-01`…`a1-21`) y `GRAMMAR_DATA.A2` pasó de 10 a **21 reglas** con ids no contiguos (`a2-01`…`a2-06`, `a2-09`…`a2-23` — hueco intencional en `a2-07`/`a2-08`, ver detalle abajo). Esto **invalida el estado "completado" de las secciones A1 y A2 de este plan** (marcadas abajo con `[x]`), que cubrían solo las 10 reglas originales de cada nivel.

**Qué cambió exactamente:**
- A1 sumó 11 reglas nuevas: `a1-11`…`a1-19` (Verben mit Vokalwechsel, Possessivartikel, Artikel interrogativ/demonstrativ, Personalpronomen Akk/Dat, Temporale Präpositionen, Hauptsätze Position 0, Komposita, Zusammengesetzte Verben, Genusregeln) + `a1-20`/`a1-21` (Verbos separables e Imperativo, **reubicados desde A2** porque corresponden a A1 en el libro de referencia).
- A2 sumó 13 reglas nuevas: `a2-11`…`a2-23` (posiciones fijas, verbos Akk+Dat, verbos con Dativ, wer/was, Präteritum sein/haben/modales, Zeitgebrauch, preposiciones locales Wohin/Wo/Woher, conectores posición 1, Nebensätze weil/wenn/dass, Partikeln+Wortbildung) y **perdió** `a2-07`/`a2-08` (movidos a A1 como `a1-20`/`a1-21` — los ids A2 no se renumeraron para no romper favoritos/SRS ya guardados por usuarios).

**Tarea pendiente:** reescribir los arrays `a1` y `a2` en `plan.js` (30 días cada uno) siguiendo el mismo patrón que se usó para B1/B2/C1/C2 (ver esas secciones abajo como plantilla): repartir las 21 reglas de cada nivel en una rotación de 30 días con primera pasada + consolidación + repaso, sin dejar ninguna regla real fuera y sin usar labels/temas inventados. Ejecutar por sesiones separadas (ideal: 1 sesión por semana, como se hizo con B1/B2) para preservar contexto.

**Otros archivos a revisar/actualizar como parte de esta tarea (verificado, no aplicado todavía):**
1. **`CLAUDE.md`** (regla de mantenimiento del propio archivo) — dos referencias desactualizadas: línea ~39 "Displays 10 key grammar rules per CEFR level" y línea ~136 "(60 rules, 10 per CEFR level A1–C2)" en la fila de `gramatica.js`. Ya no es cierto para A1 (21), A2 (21) ni B1 (31) — el total real ya no es 60. Actualizar ambas líneas al terminar esta tarea.
2. **`reformulaciones-data.json`** — el banco gratuito de `chat-reformulaciones.html` solo cubre `a1-01`…`a1-10` y `a2-01`…`a2-10` (ids originales). Tras la reubicación: las entradas guardadas bajo las claves `"a2-07"` y `"a2-08"` (separables/imperativo) quedaron huérfanas — ya no corresponden a ningún id real de `GRAMMAR_DATA` (deberían pasar a `"a1-20"`/`"a1-21"` si se quiere reaprovechar el banco existente en vez de regenerarlo). Ninguna de las 11 reglas nuevas de A1 (`a1-11`…`a1-19`) ni de las 13 nuevas de A2 (`a2-11`…`a2-23`) tiene banco: `chat-reformulaciones.html` cae automáticamente al fallback de generación IA completa (protocolo `---NUEVA---`, con coste de API) para esas reglas — funciona, pero es más caro que usar el banco. No urgente, pero si se van a repasar mucho conviene generar sus bancos con `scripts/generate-reformulaciones.js`.
3. **`gramatica.js` (`KASUS_LINKS`)** — revisado: usa `a1-06`, `a2-03`, `a2-04`, `a2-05`, `a2-06`, ninguno de esos ids se movió ni se eliminó, no requiere cambios.
4. **`plan.js` (arrays `a1`/`a2` actuales)** — revisado: no contienen deep-links por `rule.id` (el patrón `gramatica.html#{ruleId}` solo se implementó para B1, ver Backlog), así que la reubicación de ids no rompe nada existente en el plan actual; el rediseño de este punto es la propia tarea de arriba, no un efecto colateral a corregir.
...
### A1 — completado con 21 reglas (2026-07-20)
- [x] Reescrito el array `a1` completo (30 días) en `plan.js` cubriendo las 21 reglas de `GRAMMAR_DATA.A1` (`a1-01`…`a1-21`) en 4 pasadas + simulacro, mismo patrón que B1 (deep-linking por task, 2-3 reglas/día, vocabulario máx. 5 min/día, `escritura.html` y `mundliche.html` **todos los días**, no solo semana 4). Semana 1 (días 1-7): primera pasada secuencial con `gramatica.html#{id}`. Semana 2 (días 8-14): consolidación temática agrupando reglas por familia (género/artículos, pronombres/posesivos, verbos en presente, casos/der-Wörter, sintaxis, plural/preposiciones/compuestos, verbos compuestos/separables/imperativo), usando `chat-reformulaciones.html?rule={id}` para las reglas con banco (`a1-01`…`a1-10`) y `kasus.html?caso=Akkusativ&relleno=articulo` para `a1-06`. Semana 3 (días 15-21): combinaciones cruzadas entre familias. Semana 4 (días 22-30): repaso final secuencial (días 22-27), repaso libre de las 3 reglas más flojas por defecto (día 28: `a1-19,20,21`), y simulacro final días 29-30 (`escritura.html?level=A1&tipo=aleatorio` + los 3 Teile de `mundliche.html?level=A1` completos, puntuación 0-100). Vocabulario cicla las 5 categorías reales de `DataA1.json`; escritura cicla `sms→postal→email`; mündliche cicla `vorstellung→frage-antwort→bitte-formulieren`. Verificado con `node --check` y comprobación de secuencia de días (1-30 sin huecos/duplicados).
- [x] Semana 1 (histórico, versión con 10 reglas, ya reemplazada arriba):
...
### A2 — completado con 21 reglas (2026-07-20)
- [x] Reescrito el array `a2` completo (30 días) en `plan.js` cubriendo las 21 reglas de `GRAMMAR_DATA.A2` (`a2-01`…`a2-06`, `a2-09`…`a2-23`; hueco intencional en `a2-07`/`a2-08`, ver nota de reubicación arriba) en el mismo esquema de 4 pasadas + simulacro que A1/B1. Semana 1 (días 1-7): primera pasada secuencial con `gramatica.html#{id}`, salvo `kasus.html?caso=Dativ&relleno=articulo` para `a2-03` y `kasus.html?caso=Wechsel&relleno=articulo` para `a2-06`. Semana 2 (días 8-14): consolidación temática (familia Dativo, preposiciones de caso, modales+Präteritum, Perfekt+estructura, preposiciones locales wohin/wo/woher, conectores/subordinadas/partículas, comparativo/reflexivos/wer-was), usando `chat-reformulaciones.html?rule={id}` para las reglas con banco (`a2-01`…`a2-06`, `a2-09`, `a2-10`). Semana 3 (días 15-21): repaso cruzado de las mismas familias. Semana 4 (días 22-30): repaso final secuencial (días 22-27), repaso libre por defecto (día 28: `a2-21,22,23`), simulacro final días 29-30 (escritura evaluada + los 3 Teile de `mundliche.html?level=A2` completos). Vocabulario cicla las 5 categorías reales de `DataA2.json`; escritura cicla `email→nota→invitacion`; mündliche cicla `vorstellung→thema-erzaehlen→gemeinsam-planen`. Verificado con `node --check` y comprobación de secuencia de días.
- [x] Semana 1 (histórico, versión con 10 reglas, ya reemplazada arriba):
...
- [x] Deep-linking directo por tarea: cada `task.app` del plan B1 ahora apunta a la actividad concreta, no solo a la portada de la app — `gramatica.html#{ruleId}`, `B1.html?set={categoria}`, `escritura.html?level=B1&tipo={id}`, `mundliche.html?level=B1&teil={id}`, `kasus.html?caso=&relleno=`, `chat-reformulaciones.html?rule={id}`. Requirió añadir soporte de query params nuevo en `escritura.html`, `mundliche.html`, `shared-game.js` (afecta A1-C2) y `chat-reformulaciones.html` (`gramatica.html` y `kasus.html` ya lo soportaban). De paso se corrigió un bug de contenido en el día 30 (simulacro): los 3 Teile de Mündliche estaban mal etiquetados/ordenados — usaban "Sich vorstellen" (marcado en `mundliche.html` como `Extra... no forma parte del examen B1`) en vez de los 3 Teile reales del Goethe B1 (`gemeinsam-planen` → `praesentation` → `feedback-geben`); también se subió el tiempo de Präsentation en modo examen de 10 a 20 min, acorde a `vorbereitung_exam_seg: 900` (15 min) definido en `mundliche.html`.
...
- [ ] **Aplicar deep-linking directo (mismo patrón que B1) al resto de niveles.** Cuando se revisen/reescriban los planes de A1, A2, B2, C1, C2 en `plan.js`, aplicar el mismo patrón que B1: cada `task.app` debe apuntar a la actividad concreta (`gramatica.html#{ruleId}`, `{NIVEL}.html?set={categoria}`, `escritura.html?level={NIVEL}&tipo={id}`, `mundliche.html?level={NIVEL}&teil={id}`, `kasus.html?caso=&relleno=` donde aplique, `chat-reformulaciones.html?rule={id}`), no solo a la portada genérica de la app. El soporte de query params ya existe en `escritura.html`, `mundliche.html`, `shared-game.js` y `chat-reformulaciones.html` (añadido al hacer B1) — no requiere trabajo nuevo en las apps, solo reescribir los `task.app` de cada nivel.
