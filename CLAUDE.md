...
| `api/whisper.js` | Vercel serverless function — receives multipart audio, forwards to OpenAI's transcription endpoint (`/v1/audio/transcriptions`) for transcription. The model is chosen client-side in the `FormData` — all four voice apps send `gpt-4o-mini-transcribe` ($0.003/min, half of `whisper-1`, which OpenAI now lists as legacy). Requires JWT auth. Rate limited to 10 req/min per user. Pre-warms JWKS cache at module load; JWT verification and request body reading run in parallel (`Promise.all`) to reduce latency. |
...
| `api/image.js` | Vercel serverless function — receives `{ prompt, size, quality }`, calls OpenAI Images (`gpt-image-2`; siempre devuelve b64, sin `response_format` — DALL·E fue retirado de la API en mayo 2026, y `gpt-image-1-mini` se migró a `gpt-image-2` en agosto 2026 antes de su apagado del 1 dic 2026), returns `{ image_base64 }`. Requires JWT auth. Rate limited to 3 req/min per user. Used by `marketing/contenido.html`. |
