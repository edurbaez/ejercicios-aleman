...
| `admin/index.html` | Admin-only dashboard at `/admin/`. Verifies admin role on load (redirects to `/` if not admin). Shows: summary stats (total users, active last 7 days, total events), 7-day sparkline, activity by app and exam-average-by-level bar charts (each with an independent 7d/30d/Todo range pill selector, `appBarsRange`/`examStatsRange`), last-20-exams table, "Actividad últimos 5 días" per-student panel (only students with an event in the real last 5 calendar days, sorted by most recent activity first), users table with search, per-user detail panel (access control, plan-30-días progress, SRS mastery pills for vocabulary/`srs_progress` and grammar/`grammar_rule_progress` — `reps >= 2` counts as "dominada", requires the admin-read policies from migration 014 — full event log, exam table), and invite form (calls `/api/admin` with `action: 'invite'`). No navbar from main apps. |
