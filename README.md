...
- Each Teil is generated via `/api/chat` as title + situation + guide points in German, level-appropriate; the next task is prefetched in background. B1 Präsentation follows the canonical 5-"Folien" Goethe structure (Titel → Erfahrungen → Heimatland → Vor-/Nachteile + Meinung → Schluss)
...
- Static per-Teil **Redemittel** panel (collapsible, zero API cost) shown with the task; the evaluation also reports whether the student used typical exam formulas, with examples
