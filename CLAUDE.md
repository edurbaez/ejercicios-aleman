...
| `plan.html` | 30-day study plan SPA. Level selector (A1–C2), calendar grid of 30 days, progress bar, and day detail panel with task links. Progress persisted in `localStorage` per level (`plan_progress_{level}`). Uses `plan.js` for data. Blue accent (`#1565C0`). Installable PWA (`manifest-plan.json` + `sw-plan.js` + `icon-plan.svg`, "30" logo) — offline caching + optional daily reminder via periodic background sync, registered inline at the bottom of the page's script. |
