...
| `lectura veloz.html` | "Entrenamiento de lectura" (visible name; filename kept for URL stability). Speed-reading (RSVP) app: flashes words one at a time at a configurable WPM. Three mutually exclusive activities via `#activity-tabs` + `switchActivity()` — ⚡ Sprint (default on load), 📚 Lectura veloz, 📖 Comprensión; switching stops any running sprint/RSVP/blog-view TTS. Includes a "Comprensión" panel with two modes: Modo A (user pastes a German text → GPT-4o-mini generates 4 MCQs, unchanged, no Teile format) and Modo B (select CEFR level → fetches an unseen text from `reading_texts` Supabase table or generates one via IA if none available). Modo B branches on `reading_texts.format_version`: `1` (legacy — no longer generated for any level since `READING_TEILE_SPECS` now covers all six, but pre-existing rows, e.g. A1/A2 texts generated before Fase 5, still render via this flow) renders the flat single-text + 4-MCQ flow (`modoB_renderPreguntas`); `2` (all six levels A1–C2 — fases 1-5 de `lecturaplan.md`) first shows a Teil picker (`modoB_mostrarSelectorTeile()`, checkboxes over that level's Teile, all checked by default — lets the student practice only a subset instead of the full simulation) and then renders the selected Teile as a Leseverstehen simulation (`modoB_iniciarTeilSession`/`modoB_renderTeilActual`) matching the real Goethe/telc exam structure per level — A1 (3 Teile, Start Deutsch 1): richtig/falsch sobre 2 SMS/notas cortas, situaciones↔webs de información (emparejar), richtig/falsch sobre avisos públicos; A2 (4 Teile): artículo+MCQ (3 opciones), directorio/guía+MCQ (3 opciones), correo electrónico+MCQ (3 opciones), anuncios↔personas (emparejar, con un anuncio distractor); B1 (5 Teile): texto+MCQ (3 opciones), personas↔anuncios (emparejar), foro↔personas (emparejar), richtig/falsch, situaciones↔reglas (emparejar); B2 (5 Teile): personas↔afirmaciones (MCQ 4 opciones, opciones compartidas), texto con huecos (emparejar, Lückentext), texto+MCQ (3 opciones), opiniones↔titulares (emparejar), reglamento↔párrafos (emparejar); C1 (4 Teile, estructura real distinta a B1/B2 — investigada vía el Modellsatz oficial): texto con huecos léxicos (MCQ 4 opciones independientes por hueco), texto+MCQ (3 opciones), texto con huecos de frases (emparejar, Lückentext), opiniones de expertos↔afirmaciones (MCQ 4 opciones, opciones compartidas, incluye "Keiner von ihnen"); C2 (4 Teile): comentario/artículo de opinión+MCQ (4 opciones), artículo largo dividido en secciones↔resúmenes (emparejar), texto con huecos de fragmentos/frases (emparejar, Lückentext), anuncios de empleo↔perfiles de candidatos (MCQ 4 opciones, opciones compartidas). Cada Teil es respondido y luego comprobado via "Comprobar Teil" (`modoB_comprobarTeilActual`, per-type scoring: mcq/richtig_falsch compare selected `.comp-opcion`, emparejar compares `<select>` values against `teil.solucion`), then "Siguiente Teil →" advances; final screen aggregates total correct/total plus a per-Teil breakdown (`modoB_mostrarResultadoTeilFinal`). Un checkbox "⏱️ Modo examen (con tiempo)" (persistido como `comp_exam`) arranca un cronómetro con la duración real del Leseverstehen del nivel (`COMP_EXAM_MIN`, A1 25 min → C2 80 min) repartida en proporción a los ítems de los Teile elegidos (`compSegundosExamen`); al agotarse, `modoB_finalizarPorTiempo()` corrige el Teil en curso con lo respondido, cuenta 0 los Teile no abiertos y salta al resultado final. Cada sesión terminada (ambos formatos) se guarda en la tabla `reading_sessions` vía `compGuardarSesion()` y se muestra en el panel "📈 Mi progreso en comprensión" (`compRenderHistorial()`: nº de sesiones, acierto medio, mejor marca y últimas 10, reutilizando las clases `.lv-hist-*`). Display names for the Teil picker/header come from `TEIL_NOMBRES[level]`, kept in sync with `READING_TEILE_SPECS` in `api/_reading-topics.js`. Also a "⚡ Sprint de vocabulario" panel: flashes known words in its own RSVP display (source selector — SRS-mastered words with `reps >= 2` from `srs_progress` by default, system level lists A1–C2 from `word_lists`/`Data{LEVEL}.json` fallback, or the user's personal lists); every 10 words a surprise MCQ (German word → 4 Spanish options); adaptive WPM (+20 correct / −20 wrong, 80–600, persisted as `lv_sprint_wpm`); "Preguntas sorpresa" checkbox (persisted as `lv_sprint_quiz`) disables the MCQs so words just flash uninterrupted. The 📚 Lectura veloz activity itself is level-aware: initial WPM comes from `LV_WPM_POR_NIVEL[onboarding_level]` (A1 120 → C2 300) instead of a flat 400; per-chunk delay scales with average word length (`chunkDelay()`, so a Komposita gets more time than "und") on top of the punctuation pause, and the display font shrinks for long words; the old 1/2-word toggle is now a 3-state `cycleChunkMode()` (`lv_chunk_mode`) whose third mode, **Sinneinheiten**, groups up to 5 words by syntax (`chunkSizeAt()` opens a group at a preposition/conjunction from `LV_INICIO_GRUPO` and cuts on punctuation, keeping prepositional phrases and clauses intact — articles never cut). Finishing a text opens `#lv-finish` (`lvFinalizarLectura()`) with words/time/real WPM plus two actions: **📖 Comprobar comprensión** (`lvComprobarComprension()` hands the text to Comprensión Modo A and, when `compMostrarResultado()` fires, `lvAplicarComprension()` renders **velocidad efectiva** = real WPM × % correct in `#lv-efectivo`) and **🔁 Releer** (`lvLeerOtraVez()`, repeated reading at +10% WPM). Every completed pass is stored in the IndexedDB `sesiones` store (DB `lecturaVeloz` bumped to **v2**; last 100 kept, `dbSesionesPodar()`) and shown in `#lv-historial` (`lvRenderHistorial()`: total readings, mean/best effective WPM, last 10 passes). **🔍 Analizar vocabulario** (`lvAnalizarTexto()`) estimates lexical coverage of the pasted text against `LV_FUNCIONALES` + the vocabulary of the student's level and below (`sprintLevelPairs`) + their `srs_progress` words with `reps >= 2`, verdict at the 95%/90% thresholds, and lists the most frequent unknown words as chips that translate on tap (`lvTraduccion()` → `diccionario_cache`, `/api/chat` only on a miss); matching is a heuristic, not a lemmatizer (`lvEsConocida()`: root indexing, up to 3 trailing chars trimmed, `ge-…-t/-en` participles and separable-verb participles like `angerufen`). `#lv-source` brings a levelled text straight from `reading_texts` into the reader (`lvTraerTextoSistema()`, prefers rows not in `user_reading_seen`; `lvTextoPlano()` flattens `format_version` 2 rows by concatenating `questions.teile[].textos`). Saved texts now carry `level` + `words` (shown as badges, set via `showSaveTextDialog()`, synced inside `user_data.lv_texts`). Blog view defaults to `de-DE`, has a TTS rate slider (`lv_blog_rate`, default 0.85×) and every word is tappable for an inline translation. Zero API cost except Modo A/B AI calls and on-demand word translations. |
...
| `admin/index.html` | Admin-only dashboard at `/admin/`. Verifies admin role on load (redirects to `/` if not admin). Shows: summary stats (total users, active last 7 days, total events, exams, plus a 6th "Retención 8sem" card from the `marketing_summary` view — `retained_2w`/`active_8w` as a %), 7-day sparkline, activity by app and exam-average-by-level bar charts (each with an independent 7d/30d/Todo range pill selector, `appBarsRange`/`examStatsRange` — the last-20-exams table now respects `examStatsRange` too, previously a bug always showed the global last 20), "Actividad por app" `APP_NAMES` covers every app id emitted via `logEvent` including `mundliche`/`escritura`/`chatvoz2`/`auth`, "⚠️ Acceso por expirar" banner (profiles with `access_expires_at` within 7 days, hidden if none, click opens that user's detail), "Actividad últimos 5 días" per-student panel (only students with an event in the real last 5 calendar days, sorted by most recent activity first), users table with search — sortable by name/email/registro/**última actividad**/eventos, "última actividad" dot now has a 3rd red "stale" state for >30 days inactive (in addition to green "hoy"/orange "semana") — per-user detail panel (access control, plan-30-días progress, SRS mastery pills for vocabulary/`srs_progress` and grammar/`grammar_rule_progress` — `reps >= 2` counts as "dominada" — plus reading/grammar-practice completion counts from `user_reading_seen`/`user_grammar_practice_seen`, both requiring the admin-read policies from migration 015; a "📈 Actividad (últimos 30 días)" block ports the streak/%-correct/audio-minutes algorithm from `auth.js`'s `openStatsPanel()` but runs it over the already-loaded `eventsByUser` (filtered to the last 30 days) instead of a new fetch, plus a per-app summary list (`summarizeApp()`, e.g. "Chat de Voz: 12 audios / 8.4 min", "Vocabulario B1: 340 palabras · 82% acierto"); a "⏱️ Tiempo activo (últimos 30 días)" block (`renderDetTiempo()`, amber/brown `#8D6E63`) reads `daily_usage_time` (bulk-loaded in `loadData()` into `dailyTimeByUser`, requiring the admin-read policy from migration 016) and shows total active screen time plus the top 5 apps by time, via `fmtDuration()`; `APP_NAMES` also covers `a1`/`a2`/`c1`/`c2` (added for this block); full event log; exam table), and invite form (calls `/api/admin` with `action: 'invite'`). "📬 Reportes" section (`reportes-section`): header button with an unread-count badge (`renderReportsBadge()`, counts `feedback_reports` rows with `estado = 'nuevo'`, hidden at 0) scrolls to a filterable (todos/nuevo/leído/resuelto, `setReportesFilter()`) table of `feedback_reports` rows — tipo badge (🐛 bug / 💡 sugerencia), mensaje (truncated, full text in `title`), reporting user's email via `profileMap`, página, fecha, estado, and "Marcar leído"/"Marcar resuelto" buttons (`markReport()`, updates in place without a full reload). Per-user detail panel also has a "📱 Sesiones (dispositivos)" block (`renderDetSessions()`, reads `active_sessions` bulk-loaded in `loadData()` into `sessionsByUser`, requiring the admin-read policy from migration 018): lists each device with a 🟢/⚪ dot for whether it was seen in the last 2 minutes, and an amber warning banner when 2+ devices are active at once — this is how concurrent-device usage is surfaced to admins, since `auth.js`'s heartbeat only writes `last_seen`, it doesn't enforce anything. No navbar from main apps. |
...
| `gramatica.html` | Grammar rules reference SPA. Displays the key grammar rules per CEFR level (A1–C2, 21/21/31/17/17/10 respectively — see `gramatica.js` row) as an accordion. Level selection via pill buttons; hash-based routing (#a1…#c2). All content embedded in `gramatica.js`. Per-rule "🎯 Practicar" button generates a 5-question quiz from AI-generated practice sentences, cached/shared across users in `grammar_practice_exercises` (see `gramatica.js` row), de dificultad creciente dentro de la sesión. Orange theme (`#E65100`). |
| `gramaticaB1.html` | Static, JS-free single-page reference covering all 31 B1 grammar rules (same content as `GRAMMAR_DATA.B1` in `grammar-data-b1.js`, hand-transcribed — not loaded dynamically). Index of anchor links at the top, one `<section>` per rule (id `b1-01`…`b1-31`) with regla base, table, exceptions, explanation, examples and tip. No navbar, no auth, no API calls — pure HTML/CSS. Green theme (`#388E3C`), matches `B1.html`. Linked from a card on `index.html` under the "Gramática" section. |
| `gramaticaC1.html` | Static, JS-free single-page reference covering all 17 C1 grammar rules (same content as `GRAMMAR_DATA.C1` in `grammar-data-c1.js`, hand-transcribed — not loaded dynamically). Same pattern as `gramaticaB1.html`: index of anchor links at the top, one `<section>` per rule (id `c1-01`…`c1-17`) with regla base, table (where present), exceptions, explanation, examples and tip. No navbar, no auth, no API calls — pure HTML/CSS. Teal theme (`#00838F`), matches `C1.html`'s actual accent (`--color-c1` in `styles.css`; the README's PWA description calling it "Purple theme" is stale/inaccurate). Linked from a card on `index.html` under the "Gramática" section. |
| `gramaticaB2.html` | Static, JS-free single-page reference covering all 17 B2 grammar rules (same content as `GRAMMAR_DATA.B2` in `grammar-data-b2.js`, hand-transcribed — not loaded dynamically). Same pattern as `gramaticaB1.html`: index of anchor links at the top, one `<section>` per rule (id `b2-01`…`b2-17`) with regla base, table, exceptions, explanation, examples and tip. No navbar, no auth, no API calls — pure HTML/CSS. Blue theme (`#1976D2`), matches `B2.html`. Linked from a card on `index.html` under the "Gramática" section. |
| `gramaticaC2.html` | Static, JS-free single-page reference covering all 10 C2 grammar rules (same content as `GRAMMAR_DATA.C2` in `grammar-data-c2.js`, hand-transcribed — not loaded dynamically). Same pattern as `gramaticaB1.html`: index of anchor links at the top, one `<section>` per rule (id `c2-01`…`c2-10`) with regla base, optional table (only for rules whose JS entry has a `tabla` field), optional exceptions box (only `c2-09` has an `excepciones` field), explanation, examples and tip. No navbar, no auth, no API calls — pure HTML/CSS. Dark blue-gray theme (`#37474F`), matches `C2.html`. Linked from a card on `index.html` under the "Gramática" section. |
| `gramaticaA2.html` | Static, JS-free single-page reference covering all 21 A2 grammar rules (same content as `GRAMMAR_DATA.A2` in `grammar-data-a2.js`, hand-transcribed — not loaded dynamically). Index of anchor links at the top, one `<section>` per rule (id `a2-01`…`a2-23`, two ids skipped since those rules were renumbered to A1) with regla base, table, exceptions, explanation, examples and tip, same pattern as `gramaticaB1.html`. No navbar, no auth, no API calls — pure HTML/CSS. Orange theme (`#E64A19`), matches `A2.html`. Linked from a card on `index.html` under the "Gramática" section. |
| `gramaticaA1.html` | Static, JS-free single-page reference covering all 21 A1 grammar rules (same content as `GRAMMAR_DATA.A1` in `grammar-data-a1.js`, hand-transcribed — not loaded dynamically). Same structure as `gramaticaB1.html`: index of anchor links at the top, one `<section>` per rule (id `a1-01`…`a1-21`) with regla base, table, exceptions, explanation, examples and tip. No navbar, no auth, no API calls — pure HTML/CSS. Red theme (`#B71C1C`), matches `A1.html`. Linked from a card on `index.html` under the "Gramática" section. |
...
| File | Purpose |
|------|---------|
| `api/chat.js` | Vercel serverless function — proxies requests to OpenAI (`gpt-4o-mini`). Requires `Authorization: Bearer <supabase_jwt>` (verified with `SUPABASE_JWT_SECRET`). Rate limited to 20 req/min per user. Optional origin check via `ALLOWED_ORIGIN` env var and system prompt size cap (4 000 chars). Accepts an optional `temperature` (number, 0-2) forwarded to OpenAI; omitted/invalid values fall back to the OpenAI default. Pre-warms the JWKS cache at module load to reduce cold-start latency. Extra action `generate-reading` (`{ action, level }`): si `READING_TEILE_SPECS[level]` existe (B1, B2, C1 y C2), delega en `generateReadingTeile()` — genera una sesión por Teile (5 para B1/B2, 4 para C1/C2) con una llamada a GPT-4o-mini independiente por Teil, en paralelo (`Promise.allSettled`, `response_format: json_object`, `max_tokens: 2000` c/u, hasta 2 intentos por Teil vía `generateOneTeil`/`generateOneTeilAttempt` — reemplaza el diseño original de "un solo prompt para los 5 Teile", abandonado por poca fiabilidad, ver `lecturaplan.md` §12), valida cada Teil con `TEIL_VALIDATORS` (uno por tipo: `validMcqTeil`, `validRichtigFalschTeil`, `validEmparejarTeil`) dirigido por el `spec` de `READING_TEILE_SPECS[level]` (sin ninguna rama por nivel), incluido el número de ítems del Modellsatz real (`hasItemsCount`: `itemsCount` exacto, `derechaCount` como mínimo) y guarda en `reading_texts` con `format_version: 2` y `questions: { teile: [...] }`. Para los demás niveles mantiene el flujo legado (texto único + 4 preguntas MCQ, `format_version` por defecto `1`). Ambos flujos insertan con el service role (la tabla no tiene policy de INSERT para clientes) y evitan repetir título/situación consultando los últimos 8 títulos ya generados para ese nivel (y `format_version`) en `reading_texts`. Usado por el Modo B de `lectura veloz.html`. Extra action `generate-practice` (`{ action, ruleId, level, system, prompt }`): el prompt lo construye el cliente (`gramatica.js`, el contenido de la regla ya vive en `grammar-data-{level}.js` en el navegador); el endpoint solo llama a OpenAI, valida que la respuesta sea un array JSON de `{de, es}` y lo guarda en `grammar_practice_exercises` con el service role. Desde la revisión pedagógica de 2026-09-20 cada oración puede traer además `distractores[3]` (variantes de par mínimo de la misma frase, incorrectas solo en el rasgo que evalúa la regla), `por_que_mal[3]` (explicación breve de cada una) y `ordenes_validos[]` (otros órdenes de palabras igualmente correctos); el endpoint los conserva solo si vienen completos y bien formados, así que las filas antiguas sin esos campos siguen siendo válidas y el cliente cae al comportamiento anterior (`max_tokens` subió de 400 a 1500 para que quepan). Usado por el botón "🎯 Practicar" de `gramatica.html` para no regenerar oraciones que otro alumno ya generó para la misma regla. |
| `api/_reading-topics.js` | Datos compartidos por `generate-reading` en `api/chat.js` (no cuenta como función serverless propia — empieza con `_`). Exporta `TEMAS` (Themenkatalog por nivel CEFR, duplicado manualmente desde el `TEMAS` de `escritura.html`; no se comparte código porque este es un módulo ESM serverless y el otro un script de navegador), las dimensiones de aleatorización `PERSONAS`, `LUGARES`, `TONOS`, `MOMENTOS`, `CONFLICTOS`, el helper `pick()`, `READING_SPECS` (longitud/tipo de texto por nivel, flujo legado `format_version` 1) y `READING_TEILE_SPECS` (lista ordenada de Teile por nivel con su `tipo` de tarea, los seis niveles A1-C2 poblados, ver `lecturaplan.md` §11-15). Cada Teil lleva además `itemsCount`/`derechaCount` — el número de ítems y de candidatos de la columna derecha del Modellsatz real de ese nivel (A1 5/5/5, A2 5x4, B1 6/7/5/6/4, B2 9/6/6/6/3, C1 8/7/8/7, C2 10/6/6/8), interpolados en el prompt como `{n}`/`{d}` y exigidos por el validador (itemsCount exacto, derechaCount como mínimo); sustituyen a los 5 ítems fijos por Teil usados hasta 2026-09-25. |
| `api/whisper.js` | Vercel serverless function — receives multipart audio, forwards it to OpenAI's transcription endpoint (`/v1/audio/transcriptions`). **The model is forced server-side** to `gpt-4o-mini-transcribe` ($0.003/min, half of the legacy `whisper-1`): `forceModel()` parses the multipart body, drops whatever `model` field the client sent and appends its own, so a hand-rolled request can't ask for the pricier `gpt-4o-transcribe`. If the body can't be parsed it's forwarded untouched (the guard degrades instead of breaking transcription). Rejects payloads over 25 MB (OpenAI's own file limit). Requires JWT auth. Rate limited to 10 req/min per user. Pre-warms JWKS cache at module load; JWT verification and request body reading run in parallel (`Promise.all`) to reduce latency. |
...
| `api/image.js` | Vercel serverless function — receives `{ prompt, size, quality }`, calls OpenAI Images (`gpt-image-2`; siempre devuelve b64, sin `response_format` — DALL·E fue retirado de la API en mayo 2026, y `gpt-image-1-mini` se migró a `gpt-image-2` en agosto 2026 antes de su apagado del 1 dic 2026), returns `{ image_base64 }`. Requires JWT auth **and `profiles.role = 'admin'`** (`checkAccess().role`) — it's the costliest call in the project and only the admin-only `marketing/contenido.html` uses it, so the endpoint checks the role itself rather than trusting that page's gate. Unlike `checkAccess`'s usual convention this check fails **closed**: without `SUPABASE_SERVICE_ROLE_KEY` the role can't be read and nobody generates images. Rate limited to 3 req/min per user. Used by `marketing/contenido.html`. |
...
| `api/push-subscribe.js` | Vercel serverless function — manages Web Push subscriptions. GET returns current settings; POST upserts subscription + preferences (interval_hours, window_start, window_end, utc_offset_minutes); DELETE removes subscription. Requires JWT auth. Writes to `push_subscriptions` table via `SUPABASE_SERVICE_ROLE_KEY`. Extra POST action `{ action: 'notify-admins', report_id }` (rate limited 5 req/min per user, distinct from the default subscribe/upsert POST body shape): verifies the caller owns `report_id` in `feedback_reports`, looks up every `profiles.role = 'admin'` user's `push_subscriptions` row, and sends each one an immediate `web-push` notification (bypassing the interval/window logic used by `push-notify.js`'s hourly cron, since this is a one-off urgent alert, not a study reminder) — same VAPID setup and stale-subscription (410/404) cleanup as `push-notify.js`. Triggered by `auth.js`'s feedback modal right after it inserts a new `feedback_reports` row. |
...
| `api/deepseek-chat.js` | Vercel serverless function — proxies requests to DeepSeek (`deepseek-v4-flash`, con `thinking: { type: 'disabled' }` — el modo thinking por defecto consume `max_tokens` en tokens de razonamiento y a veces dejaba `content` vacío). Same structure as `chat.js`. Requires `DEEPSEEK_API_KEY`. Rate limited to 20 req/min per user. |
...
| `api/_lib.js` | Shared utilities for all serverless functions: `verifyJWT()` (ES256 via JWKS + HS256 via `SUPABASE_JWT_SECRET`), `createRateLimiter()` (Vercel KV when available, in-memory Map fallback; reads `KV_REST_API_URL`/`KV_REST_API_TOKEN` **or** the `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` pair that the Upstash Marketplace integration injects, and builds an explicit `createClient()` so either naming works), and `checkAccess()` (reads `profiles.role`/`status`/`access_expires_at` with the service role key and mirrors `is_access_valid()`; returns `role` so endpoints can gate on admin — see "Access control" below). Pre-warms JWKS cache at module load. |
...
| `auth.js` | Shared authentication module loaded by all pages. Reads `SUPA_URL`/`SUPA_KEY` globals from `config.js`. Creates `window.sb` (Supabase client), injects the login modal (email+contraseña, OTP y Google OAuth), and exposes `window.openAuthModal`, `window.signInPassword`, `window.savePassword`, `window.logout`, `window.updateAuthUI`, `window.logEvent`, `window.openStatsPanel`, `window.closeStatsPanel`, `window.getAuthToken`, `window.toggleNotifications`, `window.saveNotifSettings`, `window._setNotifInterval`. `openStatsPanel()` renders a right-side drawer with: (1) HOY cards (words answered + audios sent, same font size as the rest); (2) streak; (3) 30-day bar chart; (4) "Actividad últimos 5 días" — per-day pills by app, same weighting as `admin/index.html`'s `renderLast5Days()` (`_APP_NAMES`/`_INFRA_EVENTS` mirrored in `auth.js`, since admin doesn't load it); (5) sección "🔑 Contraseña" (`_renderPassSection()`) — crear o cambiar la contraseña de la cuenta vía `supabase.auth.updateUser({ password })` sobre la sesión ya activa; (6) sección "🔔 Recordatorios" — toggle activar/desactivar push notifications, selector de intervalo (1h/2h/3h/4h/6h) y ventana horaria (desde/hasta). La sección se renderiza via `_renderNotifSection()` que consulta `/api/push-subscribe` para cargar preferencias guardadas. Solo se muestra si `window.VAPID_PUBLIC_KEY` está configurado y el navegador soporta PushManager. **Also owns the shared "Menú ▾" dropdown**: `NAV_ITEMS` is the single source of truth for the 11 nav links + Vocabulario submenu (each entry carries `bodyId` to match `<body id="page-x">` for active-link highlighting); `_renderNavMenu()` builds the `.nav-dropdown-menu` DOM from it on `DOMContentLoaded`, before `updateAuthUI()` appends the admin-only `ADMIN_LINKS` via `_addDashboardLink()`. The 17 pages that use this nav only ship an empty `<div class="nav-dropdown-menu"></div>` — add/reorder/rename menu items only in `NAV_ITEMS`, never per-file. The 7 PWA service workers (`sw.js`, `sw-a1.js`, `sw-a2.js`, `sw-b1.js`, `sw-c1.js`, `sw-c2.js`, `sw-plan.js`) precache `auth.js`, `config.js`, and the Supabase CDN script so the JS-rendered menu still works offline. **Daily active-time tracking** (local-first, per app): a heartbeat (`_dtTick()`, every 15s while `document.visibilityState === 'visible'` and a user is signed in) accumulates ms per app into a `localStorage` key scoped to that calendar day (`ejaleman_daily_time_<YYYY-MM-DD>`, `{appId: ms}`) — zero DB calls during the day, and a day rolling over mid-session just starts writing to a new key without touching the previous day's. The app id per page comes from `document.body.id` via the `_DT_BODY_APP` map (falls back to a path-based slug for admin/teacher/marketing/chatvoz2, which have no body id); `chatvoz2/index.html`'s body id was renamed `page-cv2` (was colliding with `chat-voz.html`'s `page-cv`, which would have merged their tracked time — `styles.css`'s `#page-cv` theme rules were extended to also match `#page-cv2` so the visual theme is unaffected). On sign-in/page-load/tab-refocus, `_dtSyncStaleDays()` scans for any `ejaleman_daily_time_*` key that isn't today's, sends **one** `supabase.rpc('upsert_daily_usage_time', {p_date, p_apps})` call per stale day found, and removes the key on success — so syncing only happens on day transitions, not per session. The RPC (migration `016_daily_usage_time.sql`) is `security definer`, additively merges `apps` per key (so multiple devices syncing the same day sum instead of overwrite) and prunes rows older than 60 days for that user on every call. If the user never reopens the app, that last day's local record is lost — an accepted tradeoff (no `sendBeacon` fallback). **Feedback button** (`_injectFeedbackButton()`/`_injectFeedbackModal()`): a global fixed "💬" FAB (`#feedback-fab`, bottom-right, injected on every page on `DOMContentLoaded`) opens a modal (tipo bug/sugerencia + mensaje) — `window.submitFeedback()` inserts into `feedback_reports` (`{user_id, tipo, mensaje, pagina: location.pathname}`) via `window.sb`, then fires a fire-and-forget `POST /api/push-subscribe` with `{action: 'notify-admins', report_id}` to alert admins who have push notifications enabled. Prompts login (`window.openAuthModal()`) instead of opening the modal if no user is signed in. **Login por contraseña**: el modal ofrece email+contraseña como camino principal (`window.signInPassword()` → `signInWithPassword`), con el OTP degradado a enlace secundario ("Entrar con un código al correo"). No hay `signUp` ni flujo de reset por email a propósito: toda cuenta nace por OTP o Google (verificando el email de paso) y la contraseña se crea después desde el panel "Mi progreso"; quien la olvide entra con un código y la reescribe ahí, así que no hacen falta `resetPasswordForEmail`, una página `/reset-password` ni Redirect URLs extra en Supabase. Los errores del modal se muestran inline en `#auth-error` (`_authError()`), ya no con `alert()`. **Active session / device tracking** (concurrency detection): a stable device id (`crypto.randomUUID()`, persisted in `localStorage` as `ejaleman_device_id`, one per browser regardless of which account logs in) is upserted into `active_sessions` every 60s while the tab is visible and a user is signed in (`_asTick()`/`_asInit()`), plus once immediately on sign-in/page-load/tab-refocus (`_asForceTick()`, mirrors `_dtSyncStaleDays()`'s call sites). The device label sent (`_asDeviceLabel()`) is a short browser+OS guess parsed from `navigator.userAgent` (e.g. "Chrome · Windows"), not the raw UA string. Writes go through the `upsert_active_session(p_session_id, p_device)` RPC (migration `018_active_sessions.sql`), which also prunes that user's own devices inactive for 7+ days. Read by `admin/index.html`'s user detail panel to list devices and flag simultaneous use. |
...
| `gramatica.js` | All SPA logic for `gramatica.html`: level tab rendering, accordion toggle, and hash-based routing over the global `GRAMMAR_DATA` object (117 rules total: A1 21, A2 21, B1 31, B2 17, C1 17, C2 10 — B2/C1 follow the chapter granularity of "Grammatik aktiv B2/C1", Cornelsen) loaded from `grammar-data.js` + `grammar-data-{level}.js`. "🎯 Practicar" quiz (`getPracticeSentences()`): before calling AI, checks `grammar_practice_exercises` (Supabase, keyed by `rule_id`) for a set the current user hasn't seen yet (`user_grammar_practice_seen`, same reuse pattern as `reading_texts`/`user_reading_seen` in `lectura veloz.html`); only when every existing set for that rule was already seen does it call `/api/chat` `action: 'generate-practice'` to generate and persist `PRACTICE_EXERCISE_COUNT` (5) new sentences. Falls back to the rule's built-in `ejemplos` on any auth/network/DB error. `buildQuizQuestions()` turns the 5 sentences into 5 questions y las ordena por dificultad creciente (`QUIZ_RANK`: reconocer → completar → reordenar → producir). Los tipos dependen de si el set trae distractores de par mínimo: con ellos se usan `opcion_multiple` (las 4 opciones son la misma frase variando solo el rasgo evaluado, vía `exDistractores()`), `hueco` y `escribir_hueco` (el hueco se deriva del tramo en que los distractores difieren, `huecoDesdePares()` — el segundo se responde escribiendo) y `ordenar`; sin ellos (filas antiguas, o el fallback a `rule.ejemplos` cuando no hay sesión/red, que ahora se avisa en pantalla) se mantienen los tipos legados `opcion_multiple` / `identificar` / `ordenar` / `articulo`. `quizIsCorrect()` centraliza la comparación: `ordenar` acepta los `ordenes_validos` del set y `escribir_hueco` normaliza mayúsculas/umlauts (`examNormalize`). Al fallar, el feedback muestra el `por_que_mal` del distractor elegido en vez del `tip` genérico, y el rating SRS al cerrar el quiz es proporcional (5/5→4, 4/5→3, 3/5→2, resto 1). Also calls `/api/chat` directly (no caching) for Modo Examen's per-rule exercise generation: el prompt incluye el nivel CEFR de la regla (`ruleLevel()`) y sus temas, exige que la `instruccion` diga explícitamente cuánto se escribe y pide `respuestas_aceptadas[]`; `examCheck()` corrige tolerando mayúsculas, puntuación final, espacios y umlauts escritos ae/oe/ue/ss (avisando de la grafía exacta sin penalizar, y sin admitir alternativas en los ítems de opción múltiple), `verifyExamBatch()` resuelve cada ítem una segunda vez de forma independiente y, si discrepa, añade esa respuesta a las aceptadas en lugar de penalizar al alumno, las preguntas que no llegaron a cargarse quedan fuera de la nota, y el examen "Mixto" repasa el nivel actual y hasta tres por debajo (antes A1–B2 fijo). |
...
### Core concept
RSVP (Rapid Serial Visual Presentation): splits a text into chunks and flashes them one at a time in a large centered display at a configurable WPM. The pedagogical frame is that raw speed is meaningless without comprehension, so the activity is built around **effective reading rate** (real WPM × % comprehension), measured per pass and kept in IndexedDB.

### WPM control
`+`/`−` buttons adjust WPM in steps of 10. The starting value comes from the student's `onboarding_level` via `LV_WPM_POR_NIVEL` (A1 120 → C2 300) — a flat 400 is native-L1 speed and unreadable for a beginner in German. Per-chunk delay is `chunkDelay()`: `60000/wpm × chunk length`, scaled by average word length (0.8–2.2×, so Komposita get their time) and then by the punctuation pause (×2 after `.!?…`, ×1.5 after `,;:`).

### Chunking modes (`cycleChunkMode()`, persisted as `lv_chunk_mode`)
- `one` — one word at a time.
- `two` — two words, paired by position.
- `sinn` — **Sinneinheiten**: up to `LV_CHUNK_MAX` (5) words grouped by syntax. `chunkSizeAt()` opens a group at any preposition or conjunction in `LV_INICIO_GRUPO` and cuts on trailing punctuation; articles never cut, so determiners stay with their noun. This exists because German holds the separable prefix and the subordinate-clause verb until the end — pairing words two by two breaks exactly the parsing skill the exercise should train.

### Word display
- ORP (Optimal Recognition Point) — the focal letter of the first word is highlighted in red inside `#display`.
- Font size shrinks as the chunk grows (48px → 26px) so long compounds don't overflow the box.
- Progress bar and `Palabra N de Total` counter update on each chunk.
- Start position input lets the user jump to a specific word index before playing.

### End of pass, comprehension and history
- `lvFinalizarLectura()` closes the pass: real WPM is computed from words actually read (`lvSesion.desde` onwards) and the accumulated active time (`lvAcumularTiempo()` excludes pauses), then the pass is written to the IndexedDB `sesiones` store.
- `#lv-finish` offers **📖 Comprobar comprensión** — `lvComprobarComprension()` loads the text into Comprensión Modo A and sets `lvPendiente`; when `compMostrarResultado()` runs, `lvAplicarComprension()` writes `comprensionPct`/`wpmEfectivo` back to the stored pass and renders the effective rate in `#lv-efectivo`. `lvPendiente` is cleared by `compSwitchMode()`/`compReintentar()` so a later standalone Modo A run doesn't pick up a stale reading.
- **🔁 Releer** (`lvLeerOtraVez()`) repeats the same text at +10% WPM — repeated reading, with the history showing whether comprehension held.
- `#lv-historial` (`lvRenderHistorial()`) shows total readings, mean and best effective WPM, and the last 10 passes. Local only: no Supabase table, and `dbSesionesPodar()` keeps at most 100 rows.

### Lexical coverage (`lvAnalizarTexto()`)
Below ~95% known words fluent reading stops working, so the reader can check a text before committing to it. The known-word set (`lvPalabrasConocidas()`, cached per session, invalidated on sign-in) is `LV_FUNCIONALES` + the vocabulary of the student's level and every level below (`sprintLevelPairs`) + their `srs_progress` rows with `reps >= 2`. Verdict thresholds: ≥95% green, 90–94% orange, <90% red. The most frequent unrecognised words are rendered as chips that translate on tap.

`lvEsConocida()` is a heuristic, **not** a lemmatizer: `lvIndexarEntrada()` stores each entry plus an approximate root, `lvCoincide()` trims up to 3 trailing characters, and participles are handled both as `ge-…-t/-en` and as separable verbs (`angerufen` → `rufen`, via `LV_PARTICIPIO_SEPARABLE`). Strong irregulars (`ging` ← `gehen`) still read as unknown; the panel says the figure is an estimate.

### Levelled texts from the system (`lvTraerTextoSistema()`)
`#lv-source` pulls a text of the chosen level from `reading_texts`, preferring rows the user hasn't got in `user_reading_seen`. `lvTextoPlano()` flattens the row: `format_version` 1 uses `content`; 2 concatenates `questions.teile[].textos` (prose only — the exam tasks stay in Comprensión). Requires a signed-in user.
...
### Saved texts (IndexedDB `lecturaVeloz`, store `textos`)
- Each record is `{ id, name, text, level, words }`; `level` and `words` are set in `showSaveTextDialog()` and shown as badges in the list. `migrateFromLocalStorage()` still imports the legacy `textosguardados` key, and `mostrarTextosGuardados()` falls back to `contarPalabras()` for records saved before this field existed.
- Each saved item shows name, level, word count, preview, and Load / leer / rename / Delete buttons.
- Last reading position is stored per item as `lv_pos_<id>` in localStorage so reading resumes where left off.
- `syncToCloud()` mirrors name, text, level and words into `user_data.lv_texts` (jsonb — no migration needed for the added fields).

### Blog view
A secondary reading mode: the saved text is displayed as a full paragraph with 150-character chunks. Chunks are highlighted as TTS reads through them. Double-clicking a chunk sets the start position. A language selector (`#blog-lang`, **`de-DE` by default**) picks the TTS language and a rate slider (`#blog-rate`, persisted as `lv_blog_rate`, default 0.85×) slows the voice down — below 1× the audio accompanies the reading instead of outrunning it. Every word is wrapped by `lvChunkConPalabras()` and tapping one shows its translation in `#lv-tooltip` (`lvTraduccion()`: `diccionario_cache` first, `/api/chat` only on a miss, then written back to the cache). A thin progress bar (`#blog-progress-bar`) below the controls shows `currentChunk / totalChunks` as a percentage; updated in `highlightChunk(i)` and reset to 0 in `salirBlogView()`.

### Panel visibility
`lvMostrarPaneles(visible)` is the single place that shows/hides the Lectura veloz panels, used by both `switchActivity()` and `entrarBlogView()`/`salirBlogView()`. `LECTURA_IDS` are restored unconditionally; `LECTURA_IDS_COND` (`lv-analisis`, `lv-finish`) only reappear when they still hold content (`dataset.active === '1'`).
