...
| `gramatica.js` | All SPA logic for `gramatica.html`: level tab rendering, accordion toggle, and hash-based routing over the global `GRAMMAR_DATA` object (60 rules, 10 per CEFR level A1–C2) loaded from `grammar-data.js` + `grammar-data-{level}.js`. No external API calls. |
