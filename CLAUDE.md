...
| `teacher/index.html` | Admin/teacher-only class planner at `/teacher/`. Verifies admin role on load (redirects to `/` if not admin). Reads `teacher/clases-b1.js` (`window.TEACHER_CLASES.b1`) and cross-references rule ids against `GRAMMAR_DATA.B1` (`grammar-data.js`) to show, per week of the B1 30-day `plan.js`, the grammar content for the Tuesday and Wednesday class (fixed mapping: martes = day 2 of the week, miércoles = day 3 — a student starting the app on Monday reaches Tuesday's class already having studied day 2). Read-only in this phase: no editing UI yet for teacher-added tips/examples beyond what's in `gramatica.html`. No navbar from main apps; teal theme (`#00695C`), same as `kasus.html`. |
...
| `teacher/clases-b1.js` | Class-content mapping for B1: `window.TEACHER_CLASES.b1` — 4 week objects, each with `martes`/`miercoles` (`{ day, focus, ruleIds[] }`) referencing `GRAMMAR_DATA.B1` rule ids from `grammar-data.js`. Hand-maintained (day↔rule mapping cross-checked manually against `plan.js`). Used by `teacher/index.html`. |
