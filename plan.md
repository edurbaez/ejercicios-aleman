...
**Contexto existente:** `marketing/index.html` (estrategia), `marketing/contenido.html` (generador de carruseles con ideas/chat vía DeepSeek, export PNG con html2canvas, fondo IA opcional vía `/api/image`).

**Convención de sesiones:** cada sesión es un bloque de trabajo autocontenido que termina con commit + docs actualizadas (`CLAUDE.md` / `README.md`). Marcar `[x]` al cerrar cada una.

**Restricción:** plan Hobby de Vercel = máx 12 funciones serverless (actualmente 11/12). Ninguna propuesta debe añadir más de 1 función nueva; preferir reutilizar `/api/deepseek-chat` y acceso directo a Supabase con RLS.
...
- [ ] **Sesión 1.3 — Calendario editorial.** Nueva página `marketing/calendario.html` (misma nav-tabs): vista mensual, arrastrar/asignar piezas a fechas (`publish_date`), marcar como publicado, vista "esta semana" con pendientes. Añadir pestaña a los headers de las 3 páginas marketing.
...
### Orden de ejecución sugerido (Etapa 4)
...
| 4 — Mejoras de Marketing | Pendiente (0/15 sesiones) |
